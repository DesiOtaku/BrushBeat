...
Page {
    ...
    function handleSecond() {
        ...
        if(songInfo.currentSecond >= 30) { //switch to a new song
            ...
            songInfo.currentSecond =0;
            songInfo.songName = audioModel.get(quadCounter).title;
            if(quadCounter > 3) {
                musicPlayer.stop();
                donePlaying();
            }
    ...
    Audio {
        ...
        function nextSong() {
            musicPlayer.source = audioModel.get(quadCounter).filePath;
            musicPlayer.play();
        }
    }
    ...
    onVisibleChanged: {
        if(playFavs.visible) { //when the user can now see this page
